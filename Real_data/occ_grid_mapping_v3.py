...
MINIMUM_DISTANCE = 0.12 # lidar's minimum distance range in meters
SENSOR_RANGE = 3.0     # lidar's distance range in meters
RADIUS = 0.01 # radius of the circle to consider in the AMCL (in meters)
...
p_free = 1-p_occ
#p_occ = 0.9 # P(u-s < X < u+s) = 0.68, for X ~ N(u,s)
#p_free = 1-p_occ
...
def occupancy_grid_mapping(current_state, observed_distances, map):
    ...
    for index, observation in enumerate(observed_distances):
        if observation > MINIMUM_DISTANCE and observation < SENSOR_RANGE:
            # Location of the obstacle in (real) pixel coordinates:
            obstacle_pixel = circular_coordinates2pixel(current_state, [observation, math.radians(index)])
